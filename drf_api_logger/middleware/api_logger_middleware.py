...
class APILoggerMiddleware:
    def __init__(self, get_response):
        ...
        if hasattr(settings, "DRF_API_LOGGER_ENABLE_TRACING"):
            ...
            if self.DRF_API_LOGGER_ENABLE_TRACING and hasattr(
                settings, "DRF_API_LOGGER_TRACING_ID_HEADER_NAME"
            ):
    ...
    def __call__(self, request):
        ...
        if self.DRF_API_LOGGER_DATABASE or self.DRF_API_LOGGER_SIGNAL:
            ...
            if response.get("content-type") in (
                ...
                elif response.get("content-type") == "text/html; charset=utf-8":
                    response_body = "** html **"
                else:
                ...
                if request.user.is_authenticated:
                    # 사용자가 인증된 경우에만 email을 출력합니다.
                    request_user_id = request.user.email
                else:
                    request_user_id = "AnonymousUser"
                data = dict(
